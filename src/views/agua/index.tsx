...
const AguaPage = () => {
  ...
  const varM3 = varPct(w.consumptionM3, currentIdx)
  const varCost = varPct(w.costAmount, currentIdx)
  const costPerM3 = w.costAmount[currentIdx] && w.consumptionM3[currentIdx]
    ? Math.round(w.costAmount[currentIdx] / w.consumptionM3[currentIdx])
    : 0

  const kpis = [
    { label: 'Consumo de agua', value: w.consumptionM3[currentIdx].toLocaleString('es-CO'), unit: 'm³', icon: 'ri-drop-line', trend: varM3, color: '#2D6E8A', bg: 'rgba(45,110,138,0.10)', sub: 'vs. mes anterior' },
    { label: 'Costo hídrico', value: formatCOP(w.costAmount[currentIdx]), unit: '', icon: 'ri-money-dollar-circle-line', trend: varCost, color: '#2D6E8A', bg: 'rgba(45,110,138,0.10)', sub: 'vs. mes anterior' },
    { label: 'Costo por m³', value: costPerM3.toLocaleString('es-CO'), unit: 'COP/m³', icon: 'ri-price-tag-3-line', trend: null, color: '#2F6F57', bg: 'rgba(47,111,87,0.10)', sub: 'tarifa promedio' }
  ]
  ...
  const trendOptions: ApexOptions = {
    ...
    markers: { size: 5 },
    tooltip: { y: { formatter: (v: number) => `${v.toLocaleString('es-CO')} m³` } }
  }
  ...
  const costOptions: ApexOptions = {
    ...
    colors: w.costAmount.map((_, i) => i === currentIdx ? '#2D6E8A' : 'rgba(45,110,138,0.40)'),
    dataLabels: { enabled: false },
    legend: { show: false },
    grid: { borderColor: 'var(--mui-palette-divider)', strokeDashArray: 5, padding: { top: -10, left: 0, right: 0 } },
    xaxis: { categories: data.labels, axisBorder: { show: false }, axisTicks: { show: false }, labels: { style: { colors: 'var(--mui-palette-text-secondary)', fontSize: '12px' } } },
    yaxis: { labels: { style: { colors: 'var(--mui-palette-text-secondary)' }, formatter: (v: number) => `$${(v / 1000).toFixed(0)}k` } },
  ...
  return (
    <Grid container spacing={6}>
      ...
      <Grid item xs={12} lg={8}>
        <Card>
          <CardHeader title='Tendencia de consumo de agua' subheader={`Últimos ${data.labels.length} registros (m³)`} />
          <CardContent sx={{ pt: 0 }}>
            <AppReactApexCharts type='area' height={280} width='100%' series={[{ name: 'Consumo (m³)', data: w.consumptionM3 }]} options={trendOptions} />
          </CardContent>
        </Card>
      </Grid>
      ...
      <Grid item xs={12} lg={4}>
        <Card className='bs-full'>
          <CardHeader title='Costo hídrico' subheader='Por mes (COP)' />
          <CardContent sx={{ pt: 0 }}>
            <AppReactApexCharts type='bar' height={280} width='100%' series={[{ name: 'Costo (COP)', data: w.costAmount }]} options={costOptions} />
          </CardContent>
        </Card>
      </Grid>
      ...
      <Grid item xs={12}>
        <Card>
          <CardHeader title='Historial mensual' subheader='Agua' />
          <CardContent sx={{ pt: 0 }}>
            <TableContainer>
              <Table size='small'>
                <TableHead>
                  <TableRow>
                    {['Mes', 'Consumo (m³)', 'Costo (COP)', 'COP/m³', 'Var. consumo'].map(h => (
                ...
                <TableBody>
                  {data.longLabels.map((mes, i) => {
                    const m3 = w.consumptionM3[i]
                    const cost = w.costAmount[i]
                    const prev = i > 0 ? w.consumptionM3[i - 1] : null
                    const vp = prev ? ((m3 - prev) / prev * 100).toFixed(1) : null
                    const isCurrent = i === currentIdx

                    return (
                      <TableRow key={i} sx={{ backgroundColor: isCurrent ? 'rgba(45,110,138,0.06)' : 'transparent' }}>
                        <TableCell>
                          <Typography variant='body2' color={isCurrent ? 'text.primary' : 'text.secondary'} className={isCurrent ? 'font-semibold' : ''}>
                            {mes}{isCurrent ? ' ★' : ''}
                          </Typography>
                        </TableCell>
                        <TableCell align='right'><Typography variant='body2'>{m3.toLocaleString('es-CO')}</Typography></TableCell>
                        <TableCell align='right'><Typography variant='body2'>{formatCOP(cost)}</Typography></TableCell>
                        <TableCell align='right'><Typography variant='body2'>{m3 ? Math.round(cost / m3).toLocaleString('es-CO') : '—'}</Typography></TableCell>
